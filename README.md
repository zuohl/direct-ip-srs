...
| 文件 | 说明 |
|------|------|
| `fcm_dual.srs.json` | sing-box 规则集源码（双栈 IPv4 + IPv6） |
| `fcm_dual.srs` | 编译后的二进制规则集（sing-box 直接加载） |
| `fcm_dual.version` | 元数据（来源、生成时间、IP 数量） |
  ...
  "rule_set": [
    {
      "tag": "fcm",
      "type": "remote",
      "url": "https://raw.githubusercontent.com/<你的用户名>/fcm-srs/main/fcm_dual.srs",
      "format": "binary",
...
- 生成 `fcm_dual.srs.json`（源码）和 `fcm_dual.srs`（编译二进制）
